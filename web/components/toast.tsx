...
type Toast = { id: number; title: string; detail?: string; type: "success" | "error" };
          ...
          {toasts.map((toast) => (
            <motion.div
              ...
              className={cn(
                ...
                toast.type === "success" ? "border-l-4 border-l-green" : "border-l-4 border-l-red"
              )}
            >
              <div className="font-medium text-white">{toast.title}</div>
              {toast.detail ? <div className="mt-1 text-[11px] text-secondary">{toast.detail}</div> : null}
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
